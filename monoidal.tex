...
\label{sss:monoidal_categories}

Many types of mathematical structures have an operation which combines them to form new ones. For example: 
\begin{itemize}
...
\begin{lemma}
  The tensor product $\otimes$ of vector spaces is a bifunctor $\mathsf{Vect}_{k} \times \mathsf{Ve ct}_{k} \rightarrow \mathsf{Vect}_{k}$.
\end{lemma}
\begin{proof}
...
\begin{definition}[monoidal functor]
  ...
  Let $\mathsf{C}$ and $\mathsf{C}'$ be monoidal categories. A functor $\mathcal{F}\colon \mathsf{C} \rightarrow \mathsf{C}'$ is \defn{lax monoidal} if it is equipped with
  \begin{itemize}
...
\subsubsection{Monoidal natural transformations}
\label{sss:monoidal_natural_transformations}
...
\begin{definition}[invertible object]
  ...
  Let $(\mathsf{C}, \otimes, 1)$ be a monoidal category. An \defn{invertible object} (sometimes called a \emph{line object}) is an object $L \in \Obj(\mathsf{C})$ such that both of the functors $\mathsf{C} \rightarrow \mathsf{C}$
  \begin{itemize}
...
\begin{equation*}
  [-,[-,-]]\qquad \text{and}\quad [- \times -, -]\colon \mathsf{Set}^{\mathrm{op}} \times \mathsf{Set}^{\mathrm{op}} \times \mathsf{Set} \rightarrow \mathsf{Set}.
\end{equation*}
...
\begin{definition}[internal hom functor]
  ...
  \begin{equation*}
    {[-, -]}_{\mathsf{C}}\colon \mathsf{C}^{\mathrm{op}} \times \mathsf{C} \rightarrow \mathsf{C}
  \end{equation*}
...
We saw in \hyperref[lemma:functors_are_natural_transformations_between_hom_functors]{Lemma~\ref*{lemma:functors_are_natural_transformations_between_hom_functors}} that the any functor extends to a natural transformation between hom functors. This property extends to the internal hom.
\begin{lemma}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \to&\, \Hom_{\mathsf{C}}(R \otimes X \otimes A, R \otimes B) \\
...
\begin{proof}
  ...
  By analogy, for $f \in \Hom_{\mathsf{C}}(X, Y)$, we should define the dual morphism $f^{t} \in \Hom_{\mathsf{C}}(Y^{*}, X^{*})$ by demanding that the following diagram commutes.
  \begin{equation*}
    \begin{tikzcd}[row sep=huge, column sep=huge]
      Y^{*} \otimes X
