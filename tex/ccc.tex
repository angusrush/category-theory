...
\begin{definition}[categorical product]
  ...
  \begin{equation*}
    \begin{tikzcd}
      ...
      l & X \arrow[l, "\pi_{1}"] \arrow[r, swap, "\pi_{2}"] & B
    \end{tikzcd}
  \end{equation*}
\end{definition}
...
\begin{example}
  ...
  \begin{itemize}
    \item For any two sets, the product object $X$ is $A \times B$
...
In some categories, we can naturally take the product of any two objects; one generally says that such a category \emph{has products}. We formalize that in the following.
