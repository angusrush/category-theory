...
\section{Adjunction from universal morphism}
\label{sec:adjunction_from_universal_morphism}

\begin{definition}[universal morphism adjunction]
  \label{def:universal_morphism_adjunction}
  Let $\mathcal{F}\colon \mathsf{C} \to \mathsf{D}$ be a functor. We say that $\mathcal{F}$ is a \defn{left adjoint universal morphism adjunction} if for each $X \in \Obj(\mathsf{D})$, the category of morphisms $(\mathcal{F} \downarrow X) $(\hyperref[def:categoryofmorphismsfromafunctortoanobject]{Definition~\ref*{def:categoryofmorphismsfromafunctortoanobject}}) has a terminal object, which we denote by $\mathcal{G}(X)$. That is, $\mathcal{F}$ is a left adjoing universal morphism functor if for each $X \in \Obj(\mathsf{C})$, $Y \in \Obj(\mathsf{D})$, and morphism $f\colon \mathcal{F}(Y) \to X$, we have the following diagram.
  \begin{equation*}
    \begin{tikzcd}
      \mathcal{F}(Y)
      \arrow[rd, "f"]
  ...
  Similarly, we say that $\mathcal{F}$ is a \defn{right adjoint universal morphism adjunction} if for each $Y \in \Obj(\mathsf{C})$, the category of morphisms $(Y \downarrow \mathcal{G})$ (\hyperref[def:categoryofmorphismsfromanobjecttoafunctor]{Definition~\ref*{def:categoryofmorphismsfromanobjecttoafunctor}}) has a initial object.
\end{definition}

The above terminology is horrendous, and should probably be changed. The rationale behind introducing it is that it will not be around for long, since we will see that all of these definitions of left- and right adjunctions are equivalent.
