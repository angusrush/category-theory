...
\begin{definition}[diagram]
  \label{def:diagram}
  Let $\mathsf{J}$ and $\mathsf{C}$ be categories. A \defn{diagram} of type $\mathsf{J}$ in $\mathsf{C}$ is a functor
  \begin{equation*}
...
\end{definition}
  ...
  \hyperref[def:categoryofconesoveradiagram]{Definition~\ref*{def:categoryofconesoveradiagram}} not only gives the definition of a cone, but even prescribes what morphisms between cones look like. Let $(X, \Phi)$ be a cone over a diagram $\mathcal{D}\colon \mathsf{J} \rightarrow \mathsf{C}$, i.e.
  \begin{itemize}
...
For any functor $\mathcal{F}\colon \mathsf{C} \to \mathsf{D}$, recall that we defined the category of cones over $\mathcal{F}$ to be the category $(\Delta \downarrow \mathcal{F})$. If we pick an object $d \in \mathsf{D}$, the set of cones with tip $d$ is then $(\Delta_{d} \downarrow \mathcal{F})$.

\begin{lemma}
  \label{lemma:natural_bijection_between_cone_set_and_hom_set}
  Let $\mathcal{F}\colon I \to \mathsf{C}$ be functor. Then $\mathcal{F}$ has a limit if and only if the functor
  \begin{equation*}
    \lim_{* \in I} \Hom_{\mathsf{C}}(-, F(*))
  \end{equation*}
  is representable.
\end{lemma}
...
\begin{proof}
  ...
  \begin{equation*}
    \Hom_{\mathsf{C}}(Y, \lim_{\leftarrow}\mathcal{D}) \simeq (Y \downarrow \mathcal{D}),
  \end{equation*}
  ...
  \begin{equation*}
    \begin{tikzcd}
      ...
      \arrow[d]
      \\
      (Y \downarrow \mathcal{D})
      \arrow[r, "(-) \circ f"]
      & (Z \downarrow \mathcal{D})
    \end{tikzcd}
  \end{equation*}
  trivially commutes. We will be done if we can show that there is also a natural isomorphism
  \begin{equation*}
    (Y \downarrow \mathcal{D}) \simeq \lim_{\leftarrow} \Hom_{\mathsf{C}}(Y, \mathcal{D}).
  \end{equation*}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  commute. But this is exactly an element of $(Y \downarrow \mathcal{D})$. Conversely, every element of $(Y \downarrow \mathcal{D})$ gives us an element of $S$, so we have a bijection
  \begin{equation*}
    (Y \downarrow \mathcal{D}) \simeq \lim_{\leftarrow} \Hom_{\mathsf{C}}(Y, \mathcal{D}),
  \end{equation*}
...
Let $\mathcal{F}\colon J \to \mathcal{C}$ be any functor. The functor $\mathcal{F}$ induces a functor $\mathcal{G}\colon \mathsf{C}^{\op} \to \mathsf{Set}$ defined on objects via
\begin{equation*}
  \mathcal{G}\colon c \mapsto \lim \Hom_{\mathsf{C}}(c, F(-)).
\end{equation*}
To define $\mathcal{G}$ on morphisms, note that we can write the functor over which we are taking the limit as $h^{c} \circ F$, where $h^{c}$ is the covariant Yoneda embedding (\hyperref[note:covariantyonedaembedding]{~\ref*{note:covariantyonedaembedding}}). The functoriality of the covariant Yoneda embedding gives us, for any map $f\colon c \to d$ a natural transformation $h^{d} \circ \mathcal{F} \to h^{c} \circ \mathcal{F}$. This in turn induces a morphism of limits $\lim h^{d} \circ \mathcal{F} \to \lim h^{c} \circ \mathcal{F}$, which we define to be $\mathcal{G}(f)$.

\begin{theorem}
  Let $\mathcal{F}\colon J \to \mathsf{C}$ be a functor as above. Then the limit $\lim \mathcal{F}$ exists if and only if the functor $\mathcal{G}$ is representable.
\end{theorem}
\begin{proof}
  First, suppose that the limit $\lim \mathcal{F}$ exists. Then we have
  \begin{align*}
    \Hom_{\mathsf{C}}(-, \lim \mathcal{F}) &\simeq \lim \hom_{\mathsf{C}}(\,\cdot\,, F(-)) \\
    &\simeq \mathcal{G},
  \end{align*}
  so $\mathcal{G}$ is represented by $\lim \mathcal{F}$.

  Conversely, suppose that $\mathcal{G}$ is representable, i.e.\ there exists a natural isomorphism
  \begin{equation*}
    \mathcal{G} \simeq \Hom_{\mathsf{C}}(-, X)
  \end{equation*}
  for some $X \in \Obj(\mathsf{C})$. By definition of $\mathcal{G}$, this is a natural isomorphism
  \begin{equation*}
    \lim_{*} \Hom_{\mathsf{C}}(-, F(*)) \simeq \Hom_{\mathsf{C}}(-, X).
  \end{equation*}
  But by the above
\end{proof}
