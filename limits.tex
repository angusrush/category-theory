...
%Dually, the universal property for coproducts collapses information about maps \emph{out of} a pair of objects: we can trade a pair of maps $f\colon A \to X$ and $g\colon B \to X$ for a map $(f, g)\colon A \amalg B \to X$
%\begin{equation*}
%  \begin{tikzcd}[row sep=tiny]
%    A
%    \arrow[dr, "f"]
...
To make this statement precise, we need to clarify three things:
\begin{enumerate}
  ...
  \item We need to come up with the correct notion of maps out of such a collection into another object. This is what is known as a \emph{cone} (\hyperref[def:cone]{Definition~\ref*{def:cone}}).

  \item We need to decide what it means to say that another morphism `summarizes' such a collection. This happens when there exists a \emph{universal} cone, which we call a \emph{limit cone} (\hyperref[def:limitscolimits]{Definition~\ref*{def:limitscolimits}}).
\end{enumerate}
...
This is an abstract definition, and it will be instructive to unwind it. Let $\mathcal{D}\colon \mathsf{J} \to \mathsf{C}$ be a diagram, and let $(X, \eta)$ be a cone over $\mathsf{J}$; that is, let $\eta\colon \Delta_{X} \Rightarrow \mathcal{D}$ be a natural transformation.

The natural transformation $\eta$ provides the data of, for each object $i \in \Obj(\mathsf{J})$, a morphism
\begin{equation*}
  \eta_{i}\colon \mathcal{D}(i) \to X.
\end{equation*}

These morphisms are not arbitrary; whenever there is a morphism $f\colon i \to j$ in $\mathsf{J}$, they are required to make the diagram
\begin{equation*}
  \begin{tikzcd}[column sep=tiny]
    & X
    \arrow[dl, swap, "\eta_{i}"]
    \arrow[dr, "\eta_{j}"]
    \\
    \mathcal{D}(i)
