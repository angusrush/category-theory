...
For this reason, category theory has developed a reputation for being fairly abstract and difficult to learn, which I feel is undeserved. Categories are nothing special; they are mathematical objects, just like groups or vector spaces. They are simply good for different things.
...
Categories can be visualized as graphs, with the objects as vertices and the morphisms as edges. In fact, every category gives rise to a so-called \emph{multidigraph,} i.e.\ a directed graph which can have more than one edge between any given pair of vertices in this way.
...
\begin{notation}
  The identity morphism $1_{A}\colon A \to A$ is often simply denoted by $A$. This is actually a good notation, and we will use it freely in later chapters. However, we will avoid it in earlier chapters since it is potentially confusing whether we are talking about $A$ the object or $A$ the morphism.
\end{notation}

\begin{example}
  The prototypical category is $\mathsf{Set}$, the category whose objects are sets and whose morphisms are set functions.

  In order to check that $\mathsf{Set}$ is indeed a category, we need to check that the axioms are satisfied. In fact, everything is more or less clear by definition. Sets do indeed form a collection of objects, and between every two sets there is a collection of functions. Functions can indeed be composed, and this composititon is associative. Furthermore, every set has an identity which maps every element to itself.
\end{example}
...
The first thing we would like to define is when two objects are isomorphic.
\begin{definition}[isomorphism]\label{def:isomorphism}
  ...
  If we have an isomorphism $f\colon A \to B$, we say that $A$ and $B$ are \defn{isomorphic}, and write $A \simeq B$. This is not an abuse of notation, as the next proposition shows.
\end{definition}

\begin{proposition}\label{prop:isomorphism_is_equivalence_relation}
...
\begin{note}
  ...
  \end{itemize}
  Thus in $\mathsf{Set}$ a morphism is an isomorphism if and only if it is a monomorphism and an epimorphism. This is \emph{not} true in general: a morphism can be monic and epic without being an isomorphism.
...
\begin{definition}[small, locally small, hom-set]\label{def:smalllocallysmallcategoryhomset}
  ...
  \begin{itemize}
    ...
    \item We say that $\mathsf{C} is $\defn{locally small} if for all $A$, $B \in \Obj(\mathsf{C})$, $\Hom_{\mathsf{C}}(A, B)$ is a set. In this case, we call $\Hom_{\mathsf{C}}(A, B)$ the \defn{hom-set}. (Actually, terminology is often abused, and $\Hom_{\mathsf{C}}(A, B)$ is called a hom-set even if it is not a set.)
  \end{itemize}
\end{definition}

\section{Functors}\label{sec:functors}
Just as morphisms connect objects in the same category, functors allow us to connect different categories. In fact, functors can be viewed as morphisms in a `category of categories,' whose objects are categories!

\begin{definition}[functor]\label{def:functor}
  Let $\mathsf{C}$ and $\mathsf{D}$ be categories. A \defn{functor} $\mathcal{F}$ from $\mathsf{C}$ to $\mathsf{D}$ is the following.
  \begin{itemize}
    \item It maps each object $X \in \Obj(\mathsf{C})$ an object $\mathcal{F}(X) \in \Obj(\mathsf{D})$.

    \item It assigns each morphism $f \in \Hom(X, Y)$ to a morphism $\mathcal{F}(f)$ such that $\mathcal{F}(1_{X}) = 1_{\mathcal{F}(X)}$ for all $X$, and one of the two following properties are satisfied.
      \begin{itemize}
