...
\begin{example}\label{eg:cartesian_product_of_sets_is_bifunctor}
...
Natural transformations provide a notion of `morphism between functors.'
...
\end{equation*}
In some obvious intuitive sense, these isomorphisms are really the same isomorphism. However, it is not immediately obvious how to formalize this. They are not equal as functions, for instance, since the definition of a function contains the information about the image and coimage.
...
\begin{example}\label{eg:intertwinersarenaturaltransformations}
  ...
  Intertwiners are extremely important in high-energy physics. In relativistic quantum mechanics, the Hilbert space of a free particle alone in the world is the representation space of some irreducible representation of the Poincaré group. In scattering theory one models the state of the world as $T \to - \infty$ as a composite system of several incoming free particles, and as $T \to +\infty$ a composite system of outgoing free particles. Therefore, the Hilbert space of the incoming particles is
  \begin{equation*}
...
\begin{definition}[initial objects, final objects, zero objects]\label{def:initialfinalzeroobject}
  ...
  \begin{itemize}
    \item $A$ is said to be an \defn{initial object} if $\Hom(A,B)$ has exactly one element for all $B\in \Obj(\mathsf{C})$, i.e.\ if there is exactly one arrow from $A$ to every object in $\mathsf{C}$. Initial objects are generally called $\emptyset$.
...
Okay, so it takes a pair of objects and returns the set of morphisms between them, but any functor has to send morphisms to morphisms. How does it do that?
...
\begin{proof}
  ...
  \begin{enumerate}
    \item We can head down to $\Hom_{[\mathsf{C}^{\text{op}}, \mathsf{Set}]}(h_{B}, \mathcal{F})$, mapping
      ...
      Thus, we have
      \begin{equation*}
        (\Phi \circ \mathcal{Y}(f))(\id_{B}) = \Phi_{B}(f).
      \end{equation*}
...
\begin{definition}[unit-counit adjunction]\label{def:unitcounitadjunction}
  ...
  \begin{equation*}
    \begin{tikzcd}[column sep=huge]
      \mathsf{C}
      \arrow[r, rightsquigarrow, bend left, "\id_{\mathsf{C}}"{name=U}]
  ...
  \begin{equation*}
    \begin{tikzcd}[column sep=huge]
      ...
      \arrow[r, rightsquigarrow, swap, bend right, "\id_{\mathsf{D}}"{name=D}]
  ...
  \begin{equation*}
    \begin{tikzcd}
      ...
      \arrow[rr, rightsquigarrow, bend left=75, "\id_{\mathsf{C}}"{name=UU}]
      \arrow[rr, swap, rightsquigarrow, bend left=30, "\mathcal{G} \circ \mathcal{F}"{name=U}]
      & \mathsf{D}
      \arrow[r, rightsquigarrow, "\mathcal{G}" description]
      \arrow[rr, rightsquigarrow, bend right=75, swap, "\id_{\mathsf{D}}"{name=DD}]
