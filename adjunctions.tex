...
\begin{definition}[hom-set adjunction]
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  We say that \defn{$\mathcal{F}$ is left-adjoint to $\mathcal{G}$} (or equivalently $\mathcal{G}$ is right-adjoint to $\mathcal{F}$) if there is a natural isomorphism
  \begin{equation*}
...
\begin{proposition}
  ...
  \begin{enumerate}
    ...
    \item The functor $\mathcal{G}$ is fully faithful if and only if the counit $\epsilon\colon \mathcal{F} \circ \mathcal{G} \to \id_{\mathsf{D}}$ is a natural isomorphism
  \end{enumerate}
\end{proposition}
\begin{proof}
